...
public class Note {
	...
	/**
	 * 弃用stop和suspend
	 * stop终止所有未结束的方法，包括run方法。当线程被终止，立即释放它锁住的所有对象的锁。这会导致对象处于不一致的状态。
	 * 当线程要终止另一个线程时，无法知道什么时候调用stop是安全的。
	 * 如果用suspend挂起一个持有锁的线程，那么该锁在恢复之前是不可用的。如果调用suspend方法的的线程试图获得同一个锁，
	 * 那么程序死锁：被挂起的线程等着被恢复
	 */
}
